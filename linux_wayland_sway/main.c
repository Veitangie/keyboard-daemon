...
int main(int argc, char *argv[]) {
  ...
  if (swaysock == NULL) {
    ...
    inotify_add_watch(inotifyfd, runtimeDir,
    ...
    while (x) {
      ...
      while (total > 0) {
        if ((event->mask & IN_CREATE) > 0 &&
            strncmp(event->name, "sway-ipc.", 9) == 0) {
          free(event);
          close(inotifyfd);
          return 0;
        }
  ...
  int pipefd[2];
  if (pipe(pipefd) == -1) {
    LOG_ERR("Failed to create pipe for swaymsg\n");
    return 1;
  }

  pid_t pid = fork();
  if (pid == -1) {
    LOG_ERR("Failed to fork swaymsg process\n");
    return 1;
  } else if (pid == 0) {
    close(pipefd[0]);
  ...
  while (1) {
    ...
    if ((fds[0].revents & POLLIN) != 0) {
      int n = read(sway_fd, buf + buf_len, BUFSIZE - buf_len - 1);
      if (n > 0) {
        buf_len += n;
        buf[buf_len] = '\0';
